...
pub fn epid_report(
    ...
    let report = RaFfi::create_report(&target, data).map_err(AttestationError::CreateReportFail)?;
    let quote = RaFfi::epid_get_quote(&report, spid, sigrl.into())
        .map_err(AttestationError::GetQuoteFail)?;
